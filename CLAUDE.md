...
# Claude Code Game Studios — Agent Kit (installed)

This project has the **Claude Code Game Studios** agent kit installed in `.claude/`
(49 agents, 73 slash-command skills, 12 lifecycle hooks, 11 rules). Quick start:
**`.claude/docs/quick-start.md`**. Run **`/help`** for context-aware next steps.

## Actual stack for this project (overrides the kit's engine assumptions)
TOMB RUSH is a **web game**: React 19 + React Three Fiber (Three.js) + SpacetimeDB
(Rust module) + a Node "Tomb Keeper" LLM service. There is **no Unity / Unreal /
Godot engine** here — so the kit's ~20 engine-specialist agents (`unity-*`, `ue-*`,
`godot-*`, `unreal-specialist`) do **not** apply. Use the engine-agnostic agents
(`producer`, `game-designer`, `systems-designer`, `qa-tester`, `qa-lead`,
`ui-programmer`, `gameplay-programmer`, `narrative-director`, `sound-designer`,
`level-designer`, `economy-designer`, the directors) and the discipline skills
(`/brainstorm`, `/quick-design`, `/playtest-report`, `/balance-check`,
`/milestone-review`, `/gate-check`, `/bug-triage`, `/scope-check`, etc.).
Source lives in `client/`, `server/`, `keeper/` — not the kit's `src/`.

## Kit coordination rules + standards
