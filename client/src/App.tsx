...
import { PlayerData, InputState, Relic, BankGate, Game, Keeper, Trap, World } from './generated/types';
...
function App() {
  ...
  const registerTableCallbacks = useCallback(() => {
    ...
    conn.db.world.onInsert((_c: EventContext, w: World) => setWorld(w));
    conn.db.world.onUpdate((_c: EventContext, _o: World, w: World) => setWorld(w));
  ...
  const onSubscriptionApplied = useCallback(() => {
     ...
     if (conn) {
        ...
        const wIt = [...conn.db.world.iter()];
        setWorld(wIt.length ? wIt[0] : null);
     }
  }, []);
